...
import {createResource, JSX, Ref, createSignal, Show} from 'solid-js';
...
import SolanaActionBox from './solanaActionBox';
  ...
  return (
    props?.content && (
      ...
      >
        {props.content}
        {props.link && (
          <IconTsx icon="arrow_next" class={`${className}-footer-icon`} />
        )}
      </div>
    )
  );
...
function WebPageMedia(props: {
  ...
  return (
    <div class={`${className}-preview-resizer`}>
      {props.content || (
        <div ref={props.ref} class={classNames(_className, withDocument)}></div>
      )}
    </div>
  );
}
...
export default function WebPageBox(props: {
  ...
  const [param] = createSignal('https://jup.ag/swap/SOL-MEW');
  const [data] = createResource(param, detectSolanaAction);

  const contentDiv = (
    <div
      class={classNames(
        `${className}-content`,
        props.media?.hasDocument && 'has-document',
        props.minContent && 'min-content'
      )}
    >
  ...
  const quote = (
    <div class={classNames(`${className}-quote`, 'quote-like-border')}>
      {contentDiv}
    </div>
  );
  ...
  return (
    <Show
      when={data()}
      fallback={props.clickable ? <Ripple>{ret}</Ripple> : ret}
    >
      <SolanaActionBox {...data()} ref={props.ref} />
    </Show>
  );
}
